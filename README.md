# Установка скриптом (предпочтительно)

Требуется ubuntu 22.04 x64 чистая.

- создать сервер, прописать A записи у домена в vds.selectel.ru
- подключиться по ssh
- cкопируй из репозитория содержимое файла setup.sh
- `nano setup.sh` вставить, сохранить
- `chmod +x setup.sh`
- `./setup.sh`
- в диалоговом окне про ssh `Enter`

# Ручная установка

1. Создаем сервер-приложение "docker". Прописать A записи у домена в vds.selectel.ru
